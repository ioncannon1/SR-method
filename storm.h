...
template<class B> double spot_shape_s(const TooN::Vector<3>& x, const TooN::Vector<5, double, B>& phi, double zFactor)
{
	...
	//return -norm_sq(x - phi.template slice<2, 3>()) / (2 * phi[1] * phi[1]);
}
...
template<class B> std::tr1::tuple<double, TooN::Vector<5>, TooN::Matrix<5> > spot_shape_hess_position(const TooN::Vector<3>& x, const TooN::Vector<5, double, B>& phi, double zFactor)
{
	...
	hess[2][3] = phi[0] * es * (x[0] - phi[2])*(x[1] - phi[3]) / (r_2_pi * pow(phi[1], 5));	//YZL
	hess[3][2] = hess[2][3];
	hess[2][4] = phi[0] * es * (x[0] - phi[2])*(x[2] - phi[4]) / (r_2_pi * pow(phi[1], 5));	//YZL
	hess[4][2] = hess[2][4];

	hess[3][4] = phi[0] * es * (x[1] - phi[3])*(x[2] - phi[4]) / (r_2_pi * pow(phi[1], 5));	//YZL
	hess[4][3] = hess[4][3];
