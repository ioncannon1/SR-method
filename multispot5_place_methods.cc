...
StateParameters place_spots_intensity_sampled_old(int num_spots, const vector<Ref3D>& pixels, const vector<Image3D<float>> & ims)
{
	...
	while ((int)p.spots.size() < num_spots)
	{
		...
		if (y <= intensities[element])
			p.spots.push_back(makeVector(intensity_mode, blur_mode, pixels[element].x + rng() - .5, pixels[element].y + rng() - .5, pixels[element].z + rng() - .5));
	}
